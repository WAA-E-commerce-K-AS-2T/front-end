        ...
        <div className="flex-1 lg:w-3/4">
          <div className="border border-gray-300 rounded-lg p-6 mb-4">
            <div className="box-border flex flex-row justify-between w-full">
              <div className="box-border mt-0 ml-4 mr-4 text-xl font-semibold">Name of seller</div>
            </div>
            <h1 className="text-3xl font-bold mb-4">{productDetails1?.name}</h1>
            <div className="text-2xl font-semibold text-gray-900 mb-4">${productDetails1?.price}</div>
            <div className="box-border flex flex-wrap h-auto pb-3">
              <div className="flex items-center justify-center w-full">
                <span className="inline-flex mr-1 text-black" itemProp="ratingValue">
                  {renderStars(productDetails1?.averageRating, "black")}
                </span>
                <span className="text-lg">{productDetails1?.averageRating?.toFixed(1)}</span>
          ...
          <div>
            <h2 className="text-2xl font-semibold mb-4 cursor-pointer" onClick={() => setIsAboutCollapsed(!isAboutCollapsed)}>
              About this item
            </h2>
            {!isAboutCollapsed && <p className="text-lg text-gray-700 mb-4">{productDetails1.description}</p>}

            <hr className="border-t border-gray-300 my-4" />

            <h3 className="text-2xl font-semibold mb-4 cursor-pointer" onClick={() => setIsSpecsCollapsed(!isSpecsCollapsed)}>
              Specifications
            </h3>
            {!isSpecsCollapsed && (
          ...
          {allReviews?.map((review, index) => (
            <div key={index} className="mb-4 border border-gray-300 p-4 rounded-lg">
              <div className="flex justify-center">{renderStars(review?.rating, "yellow")}</div>
              <div className="text-lg text-gray-700">{review.comment}</div>
              <div className="text-sm text-gray-500">
                {new Date(review.createdDate).toLocaleDateString()} by {review?.createdBy}
              </div>
            </div>
          ))}
          {user?.auth_type === "buyer" ? (
